...
export function ReplacementMode() {
  const [refsText, setRefsText] = useState("1-2-3-4-2-1-5-6-2-1-2-3-7-6-3-2-1-2-3-6");
  ...
  return (
    <div className="grid grid-cols-[320px_1fr] gap-4 h-full p-4 overflow-hidden">
      <div className="space-y-3 text-xs">
        ...
        <Field label="cadena de referencias">
          <textarea
            value={refsText}
        ...
        <Field label="cantidad de marcos">
          <input
            type="number"
            min={1}
            max={10}
        ...
        <div className="border-t border-ink-800 pt-2 space-y-1 font-mono text-[11px] text-zinc-400">
          {results.map((r) => (
            <div key={r.algo} className="flex justify-between">
              ...
              <span>
                {r.faults} fallos · {(r.faultRate * 100).toFixed(1)}%
              </span>
            </div>
          ))}
        </div>
      ...
      <div className="flex flex-col gap-3 overflow-y-auto scrollbar-thin pr-1">
        {results.map((r) => (
          <div
            key={r.algo}
            className="border border-ink-800 rounded-md p-3"
          >
            ...
            <div className="overflow-x-auto scrollbar-thin">
              <table className="font-mono text-[11px]">
                <thead>
                  <tr>
                    <th className="text-left pr-3 pb-1 text-zinc-500">ref</th>
                    {r.steps.map((s, i) => (
                      <th
                        key={i}
                  ...
                  {Array.from({ length: frameCount }).map((_, frameIdx) => (
                    <tr key={frameIdx}>
                      ...
                      {r.steps.map((s, i) => (
                        <td
                          key={i}
                          className="px-1 text-center text-zinc-300"
                        >
                          {s.frames[frameIdx] ?? "·"}
                        </td>
                      ))}
